...
void CV_wafer(){
  ...
  vector<string> Date{"190808"}; 
  vector<string> Freq{"1k","10k","1M"};
  //Read data
  double Vr, Cr;
  string buffer;
  map< array<int,4>,vector<double>> V;
  map< array<int,4>,vector<double>> C;
  array<int,4> key;
  vector<double> VV;
  vector<double> CC;
  for(int type=0;type<Celltype.size();++type){
        for(int date=0;date<Date.size();++date){
      for(int wafer=0;wafer<1;++wafer){
        for(int fq=0;fq<Freq.size();++fq){
          sprintf(Filename,"/home/kycheng/Silicon_Sensor/HGcal_Sensor/%s_NCU/KEI_2410/HGcal_6inch_%02d/%s/cv%s.txt",Date[date].c_str(), Wafer[wafer], Celltype[type].c_str(), Freq[fq].c_str());
          //cout << Filename << endl;
          fstream r1;
          ...
          CC.clear();
          if(r1){
            key={type,date,wafer,fq};
            getline(r1,buffer);
            r1>>Vr;
            while(r1>>buffer>>buffer>>buffer>>Vr>>buffer>>buffer>>Cr){
              VV.push_back(Vr);
              CC.push_back(Cr);
              cout << Vr << " " << Cr << endl;
            }
